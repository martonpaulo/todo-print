...
const stubLayout = ({
  ...
  vi.spyOn(Element.prototype, "getBoundingClientRect").mockImplementation(
    function (this: Element) {
      const measured =
        this instanceof HTMLElement && this.dataset.measureList !== undefined
          ? listHeight
          : panelHeight;
...
const removeTask = async (
  ...
  await user.click(
    screen.getByRole("button", {
      name: COPY.removeTaskLabel(taskContext, listContext),
    }),
  );
  await user.click(
    screen.getByRole("button", {
      name: COPY.confirmRemoveTaskLabel(taskContext, listContext),
    }),
  );
};
...
const removeList = async (
  ...
  await user.click(
    screen.getByRole("button", { name: COPY.removeListLabel(listContext) }),
  );
  await user.click(
    screen.getByRole("button", {
      name: COPY.confirmRemoveListLabel(listContext),
    }),
  );
};
...
describe("App", () => {
  ...
  it("names the editor and preview regions from the centralized copy", () => {
    ...
    expect(
      screen.getByRole("region", { name: COPY.editorRegion }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: COPY.previewRegion }),
    ).toBeInTheDocument();
  });
  ...
  it("leads from the blocked print action to the first oversized list", async () => {
    ...
    const blockedPrint = await screen.findByRole("button", {
      name: COPY.printBlocked,
    });
    ...
    await user.click(
      screen.getByRole("button", { name: COPY.goToOverflowList }),
    );

    const firstList = screen.getByRole("region", {
      name: `List 1: ${COPY.starter.priorities}`,
    });
  ...
  it("offers no jump while every list fits", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: COPY.goToOverflowList }),
    ).not.toBeInTheDocument();
  });
  ...
  it("moves to the preview and back to the last edited control", async () => {
    ...
    const title = screen.getByRole("textbox", {
      name: `Title of List 2: ${COPY.starter.smallWins}`,
    });
  ...
  it("marks the stacked-layout navigation as revealed only at the breakpoint", () => {
    ...
    expect(screen.getByRole("button", { name: COPY.goToPreview })).toHaveClass(
      "narrow-only",
    );
  ...
  it("persists the Moon type setting with the document", async () => {
    ...
    await user.click(
      screen.getByRole("checkbox", { name: COPY.moonTypography }),
    );
  ...
  it("persists the print rotation and declares the portrait sheet only while it is on", async () => {
    ...
    expect(
      JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "{}").rotatePrint,
    ).toBe(true);

    cleanup();
    render(<App />);
    expect(
      screen.getByRole("checkbox", { name: COPY.rotatePrint }),
    ).toBeChecked();
  ...
  it("describes what the Moon type toggle changes", () => {
    ...
    expect(
      screen.getByRole("checkbox", { name: COPY.moonTypography }),
    ).toHaveAccessibleDescription(COPY.moonTypographyHint);
  });
  ...
  it("marks the document unsaved when the browser refuses the write, then clears it once a write succeeds", async () => {
    ...
    const title = screen.getByRole("textbox", {
      name: `Title of List 1: ${COPY.starter.priorities}`,
    });
    ...
    expect(
      screen.queryByText(COPY.saveFailedDescription),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps unreadable stored content until the user replaces it explicitly", async () => {
    ...
    expect(
      screen.getByRole("alert", { name: COPY.loadFailedTitle }),
    ).toHaveTextContent(COPY.loadFailedDescription);
    ...
    await user.click(
      screen.getByRole("button", { name: COPY.replaceStoredDocument }),
    );
    await user.click(
      screen.getByRole("button", { name: COPY.confirmReplacement }),
    );
  ...
  it("stops claiming a save while an invalid Markdown draft is unparsed", async () => {
    ...
    expect(screen.getByRole("status")).toHaveTextContent(COPY.savedLocally);
    expect(
      screen.queryByText(COPY.draftNotSavedDescription),
    ).not.toBeInTheDocument();
  });
  ...
  it("recovers a removed task through the visible Undo action", async () => {
    ...
    expect(
      screen.queryByRole("textbox", {
        name: `${taskContext} in ${listContext}`,
      }),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("button", { name: COPY.undoRemoval }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: COPY.editorRegion }),
    ).toHaveFocus();
  ...
  it("restores one task per undo when two removals follow each other", async () => {
    ...
    expect(
      screen.getByRole("textbox", {
        name: taskName(1, COPY.starter.priorityItems[1]),
      }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("textbox", {
        name: taskName(1, COPY.starter.priorityItems[0]),
      }),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByRole("textbox", {
        name: taskName(2, COPY.starter.priorityItems[1]),
      }),
    ).toBeInTheDocument();
  });
  ...
  it("recovers a removed list through the same visible action", async () => {
    ...
    expect(
      screen.getByRole("region", { name: listContext }),
    ).toBeInTheDocument();
  });
  ...
  it("undoes and redoes a removal from the keyboard outside a text field", async () => {
    ...
    expect(
      screen.queryByRole("textbox", { name: taskName }),
    ).not.toBeInTheDocument();
  ...
  it("abandons the redo branch when an edit follows an undo", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: COPY.addTaskLabel(listContext) }),
    );
    fireEvent.keyDown(window.document.body, {
      key: "z",
      ctrlKey: true,
  ...
  it("downloads the document as a Markdown file named after its date", async () => {
    ...
    const exportButton = screen.getByRole("button", {
      name: COPY.exportMarkdown,
    });
    await user.click(exportButton);

    const link = click.mock.instances[0] as HTMLAnchorElement;
    expect(link.download).toMatch(/^todo-\d{4}-\d{2}-\d{2}\.md$/);
    await expect(blobs[0].text()).resolves.toContain(
      `## ${COPY.starter.priorities}`,
    );
  ...
  it("replaces the document with an imported Markdown file, and undo restores it", async () => {
    ...
    expect(
      screen.getByRole("textbox", {
        name: "Task 1: From a file in List 1: Imported",
      }),
    ).toBeInTheDocument();
  ...
  it("gives an import its own undo step even when edits surround it in the same instant", async () => {
    ...
    const listContext = COPY.listContext(1, COPY.starter.priorities);
    fireEvent.change(
      screen.getByRole("textbox", { name: `Title of ${listContext}` }),
      {
    ...
    fireEvent.change(
      screen.getByRole("textbox", { name: "Title of List 1: Imported" }),
      {
  ...
  it("keeps the document unchanged when the imported Markdown has errors", async () => {
    ...
    expect(
      screen.queryByText(COPY.importFailedWithErrors),
    ).not.toBeInTheDocument();
  });
  ...
  it("drops the invalid-import status once the imported source is edited", async () => {
    ...
    expect(
      screen.getByText(COPY.markdownErrorLine(3, "unrecognized-line")),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(COPY.importFailedWithErrors),
    ).not.toBeInTheDocument();
  ...
  it("reports a file the browser cannot read without touching the document", async () => {
    const user = userEvent.setup();
    vi.spyOn(File.prototype, "text").mockRejectedValue(
      new Error("not readable"),
    );
    ...
    expect(
      screen.getByRole("textbox", {
        name: `Title of ${COPY.listContext(1, COPY.starter.priorities)}`,
      }),
  ...
  it("restores a valid stored document without any recovery prompt", () => {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        blocks: [{ id: "list-1", kind: "list", title: "Restored", items: [] }],
      }),
    );
