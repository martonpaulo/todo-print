...
const documentOfPanels = (panelCount: number): TodoDocument => ({
  ...
  blocks: Array.from(
    { length: panelCount },
    (_, index) => index,
  ).flatMap<DocumentBlock>((index) => [
    ...(index === 0
      ? []
      : [{ kind: "panel-break" as const, id: `break-${index}` }]),
    {
      kind: "list" as const,
      id: `list-${index + 1}`,
      title: `Panel ${index + 1}`,
...
const renderPanels = (panelCount: number) => {
  const { container } = render(
    <PrintPreview
      document={documentOfPanels(panelCount)}
...
describe("PrintPreview page structure", () => {
  ...
  it.each([1, 2, 3, 4])(
    "gives a %i-panel document whole sheets of three slots",
    (panelCount) => {
      const { pages } = renderPanels(panelCount);

      expect(pages).toHaveLength(Math.ceil(panelCount / PANELS_PER_PAGE));
      pages.forEach((page) => {
        expect(page.querySelectorAll(".print-panel")).toHaveLength(
          PANELS_PER_PAGE,
        );
      });
      expect(
        pages.flatMap((page) => [
          ...page.querySelectorAll(".print-panel--filler"),
        ]),
      ).toHaveLength(pages.length * PANELS_PER_PAGE - panelCount);
    },
  );
  ...
  it("numbers only the real panels and dates only the first", () => {
    ...
    const numbers = pages
      .flatMap((page) => [...page.querySelectorAll(".print-panel-number")])
      .map((element) => element.textContent);

    expect(numbers).toEqual(
      [1, 2, 3, 4].map((panel) => COPY.panelNumber(panel, 4)),
    );
    expect(
      pages.flatMap((page) => [...page.querySelectorAll(".print-date")]),
    ).toHaveLength(1);
  ...
  it("declares no page width of its own, so the print tokens stay the only owner", () => {
    ...
    pages.forEach((page) => {
      expect(page.getAttribute("style") ?? "").not.toMatch(
  ...
  it("renders tasks without a heading or divider when a list title is blank", () => {
    ...
    if (list?.kind !== "list") throw new Error("The fixture rendered no list");
    list.title = "   ";
...
describe("PrintPreview zoom", () => {
  ...
  const renderPreview = (onLayoutStatusChange = () => {}) =>
    render(
      <PrintPreview
        document={documentOfPanels(1)}
        onLayoutStatusChange={onLayoutStatusChange}
      />,
    );

  const zoomLevel = () =>
    screen.getByRole("group", { name: COPY.previewZoom }).textContent;
