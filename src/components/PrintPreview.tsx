 ...
 */
const PrintList = memo(
  ({ list, typography, overflowing = false }: PrintListProps) => (
    <section
      className={`print-list${overflowing ? " print-list--overflow" : ""}`}
    >
      {list.title.trim() !== "" && (
        <>
...
const PanelHeader = ({
  ...
  <header className="print-panel__header">
    ...
    {document.showPanelNumbers && (
      <p className="print-panel-number">
        {COPY.panelNumber(panelIndex + 1, panelCount)}
      </p>
    )}
  </header>
);
 ...
 */
const FillerPanel = () => (
  <div className="print-panel print-panel--filler" aria-hidden="true" />
);
...
const MeasurementLayer = ({
  ...
  const lists = document.blocks.filter(
    (block): block is ListBlock => block.kind === "list",
  );
...
const measurementsMatch = (
  left: MeasurementState,
  right: MeasurementState,
): boolean => {
  if (
...
const usePrintMeasurements = (document: TodoDocument) => {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      root
        .querySelectorAll<HTMLElement>("[data-measure-list]")
        .forEach((element) => {
      ...
      const measuredFirstPanel =
        root
          .querySelector<HTMLElement>("[data-capacity-first]")
          ?.getBoundingClientRect().height ?? 0;
      const measuredPanel =
        root
          .querySelector<HTMLElement>("[data-capacity-regular]")
          ?.getBoundingClientRect().height ?? 0;
      const listGap =
        root
          .querySelector<HTMLElement>("[data-measure-gap]")
          ?.getBoundingClientRect().width ?? 0;
      const listCount = document.blocks.filter(
      ...
      const next = {
        ...
        // result as final.
        firstPanelCapacity: Math.max(
          0,
          measuredFirstPanel - MEASUREMENT_SAFETY_PX,
        ),
      ...
      recordProfileSample(
        "print-measurement",
        performance.now() - measurementStart,
      );
...
const clampZoom = (zoom: number): number =>
  Math.min(
    ZOOM_MAX,
    Math.max(ZOOM_MIN, Math.round(zoom / ZOOM_STEP) * ZOOM_STEP),
  );
...
const PreviewZoomControls = ({
  ...
  return (
    <div
      className="preview-zoom screen-only"
      role="group"
...
export const PrintPreview = ({
  ...
  const changeZoom = useCallback(
    (next: number) => setZoom(clampZoom(next)),
    [],
  );
  ...
  const pages = useMemo(
    () => groupPanelsIntoPages(layout.panels),
    [layout.panels],
  );
  ...
  return (
    <>
      ...
      <div className="preview-stage" ref={stageRef}>
        <section className="print-pages" aria-label={COPY.previewTitle}>
