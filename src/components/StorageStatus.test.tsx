...
const Harness = ({
  ...
  resultingStatus: PersistenceStatus;
}) => {
...
describe("StorageStatus", () => {
  ...
  it("announces a load failure assertively, takes focus, and offers the explicit replacement", async () => {
    ...
    render(
      <StorageStatus
        status="load-failed"
        hasUnsavedDraft={false}
        onReplaceStoredDocument={onReplaceStoredDocument}
      />,
    );
    ...
    await userEvent.click(
      screen.getByRole("button", { name: COPY.cancelReplaceStoredDocument }),
    );
    expect(onReplaceStoredDocument).not.toHaveBeenCalled();
  ...
  it("stays out of the printed page", () => {
    ...
    expect(container.querySelector(".storage-status")).toHaveClass(
      "screen-only",
    );
  });
});
