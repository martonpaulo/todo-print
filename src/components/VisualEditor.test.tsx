...
describe("VisualEditor accessible names", () => {
  ...
  it("names every list section by position, including duplicate and empty titles", () => {
    ...
    expect(
      screen.getByRole("region", { name: "List 1: Work" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: "List 2: Work" }),
    ).toBeInTheDocument();
  ...
  it("gives every repeated list control the affected list as context", () => {
    ...
    expect(
      screen.getByRole("textbox", { name: "Title of List 2: Work" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Move List 2: Work up" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Move List 2: Work down" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Remove List 2: Work" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Add task to List 2: Work" }),
    ).toBeInTheDocument();
  });
  ...
  it("gives every task control both task and list context, with a position fallback", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Remove Task 1: Draft from List 1: Work",
      }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("checkbox", {
        name: "Mark Task 1: Draft in List 2: Work complete",
      }),
    ).toBeInTheDocument();

    // An empty task falls back to its position instead of an empty name.
    expect(
      screen.getByRole("textbox", { name: "Task 2 in List 1: Work" }),
    ).toBeInTheDocument();
  });
  ...
  it("keeps the first list move-up and last block move-down buttons disabled", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Move List 1: Work up" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "Move List 3 down" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "Move List 2: Work up" }),
    ).toBeEnabled();
  ...
  it("keeps focus on the edited title while its contextual name changes", async () => {
    ...
    rerender(
      <VisualEditor document={next} overflowListIds={[]} onChange={onChange} />,
    );

    const renamed = screen.getByRole("textbox", {
      name: /^Title of List 1: /u,
    });
  ...
  it("creates a list with an empty title and a positional accessible name", async () => {
    ...
    expect(
      screen.getByRole("textbox", { name: "Title of List 4" }),
    ).toHaveValue("");
  });
});
...
describe("VisualEditor checkbox target", () => {
  ...
  it("leaves the task text field and remove button outside the checkbox target", async () => {
    ...
    const text = screen.getByRole("textbox", {
      name: "Task 1: Draft in List 1: Work",
    });
    ...
    await user.click(
      screen.getByRole("button", {
        name: "Confirm removing Task 1: Draft from List 1: Work",
      }),
    );

    const next = onChange.mock.calls.at(-1)?.[0] as TodoDocument;
    const list = next.blocks[0];
    expect(list.kind === "list" && list.items.map((item) => item.id)).toEqual([
      "item-a2",
    ]);
  });
});
...
describe("VisualEditor destructive confirmation", () => {
  ...
  it("leaves the document untouched until a list removal is confirmed", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Remove List 1: Work" }),
    );
    expect(onChange).not.toHaveBeenCalled();
    ...
    expect(
      screen.getByRole("group", { name: COPY.confirmRemoveList }),
    ).toBeInTheDocument();
    const confirm = screen.getByRole("button", {
      name: "Confirm removing List 1: Work",
    });
  ...
  it("lands on the previous list when the last one is removed", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Confirm removing List 3" }),
    );
  ...
  it("lands on the add-list action when the only list is removed", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Confirm removing List 1: Work" }),
    );

    // Nothing is left in the stack to hold focus, so the action that rebuilds
    // a list is the landing point.
    await waitFor(() =>
      expect(screen.getByRole("button", { name: COPY.addList })).toHaveFocus(),
    );
  });
  ...
  it("keeps the list and returns focus to its remove control when declined", async () => {
    ...
    expect(
      screen.queryByRole("group", { name: COPY.confirmRemoveList }),
  ...
  it("asks before removing a task that holds text, and only that task", async () => {
    ...
    expect(onChange).not.toHaveBeenCalled();
    expect(
      screen.getByRole("group", { name: COPY.confirmRemoveTask }),
    ).toBeInTheDocument();
    // The question replaces only its own control; every other row keeps one.
    expect(
      screen.getByRole("button", {
        name: "Remove Task 1: Draft from List 2: Work",
      }),
    ).toBeInTheDocument();
  ...
  it("keeps the task and returns focus to its remove control when declined", async () => {
    ...
    await user.click(
      screen.getByRole("button", {
        name: "Keep Task 1: Draft in List 1: Work",
      }),
    );
  ...
  it("removes an empty task without a question", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Remove Task 2 from List 1: Work" }),
    );

    expect(
      screen.queryByRole("group", { name: COPY.confirmRemoveTask }),
    ).not.toBeInTheDocument();
    const next = onChange.mock.calls.at(-1)?.[0] as TodoDocument;
    const list = next.blocks[0];
    expect(list.kind === "list" && list.items.map((item) => item.id)).toEqual([
