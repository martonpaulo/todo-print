...
const MARKDOWN_ERROR_MESSAGES = {
  ...
  "unsupported-heading":
    "Use exactly two hashes and a space, as in “## List title”.",
...
export const COPY = {
  ...
  importFailed:
    "Could not read that file. Choose a Markdown (.md) file saved from this editor.",
  ...
  panelBreakDescription:
    "Everything below starts on a fresh 99 × 210 mm panel.",
  ...
  overflowDescription:
    "Shorten the highlighted list before printing. Lists are never split.",
  ...
  listOverflow:
    "This list is too tall for one panel. Shorten it before printing.",
  ...
  panelNumber: (current: number, total: number) =>
    `Panel ${current} / ${total}`,
  ...
  taskCompleteLabel: (task: string, list: string) =>
    `Mark ${task} in ${list} complete`,
  removeTaskLabel: (task: string, list: string) =>
    `Remove ${task} from ${list}`,
  confirmRemoveTaskLabel: (task: string, list: string) =>
    `Confirm removing ${task} from ${list}`,
  cancelRemoveTaskLabel: (task: string, list: string) =>
    `Keep ${task} in ${list}`,
