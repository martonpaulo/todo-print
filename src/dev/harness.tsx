...
const buttonByTitle = (title: string, index = 0): HTMLButtonElement => {
  ...
  if (!button)
    throw new Error(`Harness could not find the "${title}" button #${index}`);
  return button;
};
...
const buttonByText = (text: string): HTMLButtonElement => {
  ...
  if (!button)
    throw new Error(`Harness could not find a button labelled "${text}"`);
  return button;
};
...
const scenarios: Scenario[] = [
  ...
  },
  {
    ...
    step: (iteration) => {
      ...
      buttonByTitle(
        moveDown ? COPY.moveListDown : COPY.moveListUp,
        moveDown ? 0 : 1,
      ).click();
    },
  },
  {
    ...
    step: () => {
      require$<HTMLInputElement>(
        '.document-toolbar input[type="checkbox"]',
      ).click();
    },
  },
  {
    ...
    step: (iteration) => {
      const textarea = require$<HTMLTextAreaElement>(
        ".editor-scroll-region textarea",
      );
      setNativeValue(
        textarea,
        `${textarea.value}${"abcdefghij"[iteration % 10]}`,
      );
    },
...
const runScenario = async (
  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    ...
    samples.push({
      ...
      editToLastCommit:
        lastCommitTaskEnd > 0 ? lastCommitTaskEnd - start : handlerReturn,
      handlerReturn,
  ...
  const pick = (
    selector: (sample: Sample) => number,
    fraction: number,
  ): number => round(percentile(samples.map(selector), fraction));
  ...
  return {
    ...
    printMeasurementCountP95: pick(
      (sample) => sample.printMeasurementCount,
      0.95,
    ),
...
const waitForLayoutReady = async (listCount: number): Promise<void> => {
  ...
  throw new Error(
    "Harness timed out waiting for the print preview to become ready",
  );
};
