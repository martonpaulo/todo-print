...
const CASES: ReadonlyArray<{ value: string; valid: boolean; reason: string }> =
  [
    ...
    { value: "2024-02-29", valid: true, reason: "a leap day in a leap year" },
    {
      value: "2000-02-29",
      valid: true,
      reason: "a leap day in a century leap year",
    },
    ...
    { value: "2026-12-31", valid: true, reason: "the last day of a year" },
    {
      value: "9999-12-31",
      valid: true,
      reason: "the last representable four-digit date",
    },
    {
      value: "0001-01-01",
      valid: true,
      reason: "a year below the two-digit boundary",
    },
    {
      value: "0050-03-01",
      valid: true,
      reason: "a year the 1900 offset would misread",
    },
    {
      value: "2026-02-29",
      valid: false,
      reason: "a leap day outside a leap year",
    },
    {
      value: "1900-02-29",
      valid: false,
      reason: "a leap day in a non-leap century year",
    },
    {
      value: "2026-04-31",
      valid: false,
      reason: "a day past the end of a 30-day month",
    },
...
describe("the ISO date invariant", () => {
  it.each(CASES)(
    "reports $value as $valid, being $reason",
    ({ value, valid }) => {
  ...
  it.each(
    CASES.filter(
      (testCase) =>
        testCase.value !== "" && testCase.value === testCase.value.trim(),
    ),
  )("agrees at both input boundaries for $value", ({ value, valid }) => {
