...
describe("Markdown document conversion", () => {
    ...
    const document: TodoDocument = {
      ...
      blocks: [
        ...
        },
        {
          ...
          title: "",
          items: [
            { id: "item-adjacent", text: "Adjacent task", checked: false },
          ],
        },
        { id: "break-1", kind: "panel-break" },
        {
  ...
  it("reports unsupported content instead of silently discarding it", () => {
    ...
    expect(result.errors).toEqual([{ line: 2, code: "unrecognized-line" }]);
  });
  ...
  it.each([
    ...
    "rejects a heading with $label instead of reinterpreting it",
    ({ source }) => {
  ...
  it("carries the print rotation through a Markdown round trip without serializing it", () => {
    const rotatedDocument: TodoDocument = {
      ...baseDocument,
      rotatePrint: true,
    };
...
describe("Markdown draft parsing and identity reconciliation", () => {
  ...
  it("parses an ID-free draft before reconciliation", () => {
    const result = parseMarkdownDraft(
      "# 2026-08-24\n\n## List\n* [x] Done\n\n---",
    );
...
describe("Markdown source editing", () => {
  ...
  it("preserves indentation and CRLF when continuing a checklist", () => {
    ...
    expect(
      continueMarkdownAtSelection(source, source.length, source.length),
    ).toEqual({
      source: "## List\r\n  * [x] Task\r\n  - [ ] ",
