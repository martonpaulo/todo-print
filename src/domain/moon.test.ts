...
describe("toMoonSegments", () => {
  it("turns a run of letters into one word segment", () => {
    ...
    expect(
      segments[0].kind === "moon" && segments[0].glyphs.map((g) => g.letter),
    ).toEqual(["P", "L", "A", "N"]);
  });
  ...
  it("keeps whitespace and unmapped characters as plain runs, in order", () => {
    expect(
      toMoonSegments("Pay 12 bills!").map((segment) =>
        segment.kind === "moon"
          ? ["moon", segment.source]
          : ["plain", segment.text],
      ),
  ...
  it("loses no character of the source", () => {
    ...
    const rebuilt = toMoonSegments(source)
      .map((segment) =>
        segment.kind === "moon" ? segment.source : segment.text,
      )
      .join("");
  ...
  describe("accented words", () => {
    ...
    it("keeps an accented word one uninterrupted Moon run", () => {
      for (const word of [composed, decomposed]) {
        ...
        expect(new Set(segments.map((segment) => segment.kind)), word).toEqual(
          new Set(["moon"]),
        );
        expect(
          segments.flatMap((segment) =>
            segment.kind === "moon"
              ? segment.glyphs.map((glyph) => glyph.letter)
              : [],
          ),
          word,
  ...
  describe("bounded break opportunities", () => {
    ...
    it("never emits a chunk wider than the bound", () => {
      for (const segment of toMoonSegments(`${longRun} ${longRun}`)) {
        if (segment.kind === "moon") {
          expect(segment.glyphs.length).toBeLessThanOrEqual(
            MOON_MAX_RUN_GLYPHS,
          );
        }
      }
    ...
    it("splits a long run into chunks that reconstruct it exactly", () => {
      ...
      expect(chunks).toHaveLength(
        Math.ceil(longRun.length / MOON_MAX_RUN_GLYPHS),
      );
      expect(
        chunks
          .map((segment) => (segment.kind === "moon" ? segment.source : ""))
          .join(""),
      ).toBe(longRun);
    });
    ...
    it("marks every chunk after the first as continuing its run", () => {
      ...
      expect(
        chunks.map(
          (segment) => segment.kind === "moon" && segment.continuesRun,
        ),
      ).toEqual([false, ...Array(chunks.length - 1).fill(true)]);
    });
