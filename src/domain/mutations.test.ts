...
const blockIds = (document: TodoDocument) =>
...
const itemsOf = (document: TodoDocument, listId: string): TodoItem[] => {
  ...
  if (block?.kind !== "list") throw new Error(`missing list ${listId}`);
  return block.items;
};
...
describe("removeBlock", () => {
  ...
  it("keeps every block when the id is unknown", () => {
    ...
    expect(blockIds(removeBlock(document, "list-missing"))).toEqual(
      blockIds(document),
    );
  });
});
...
describe("updateListTitle", () => {
  ...
  it("changes nothing when the list is unknown", () => {
    ...
    expect(updateListTitle(document, "list-missing", "Renamed").blocks).toEqual(
      document.blocks,
    );
  });
});
...
describe("updateItemText and setItemChecked", () => {
  ...
  it("toggles the checked state without touching the text", () => {
    ...
    expect(itemsOf(next, "list-a")[0]).toEqual({
      id: "item-a1",
      text: "Draft",
      checked: true,
    });
  ...
  it("changes nothing when the item is unknown", () => {
    ...
    expect(
      itemsOf(
        updateItemText(document, "list-a", "item-missing", "x"),
        "list-a",
      ),
    ).toEqual(itemsOf(document, "list-a"));
  });
});
...
describe("insertItemAfter", () => {
  ...
  it("changes nothing when the list is unknown", () => {
    ...
    expect(insertItemAfter(document, "list-missing", newItem).blocks).toEqual(
      document.blocks,
    );
  });
});
...
describe("removeItem", () => {
  ...
  it("changes nothing when the item is unknown", () => {
    ...
    expect(
      itemsOf(removeItem(document, "list-a", "item-missing"), "list-a"),
    ).toEqual(itemsOf(document, "list-a"));
  });
});
