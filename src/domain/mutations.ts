...
export const appendBlock = (
  document: TodoDocument,
  block: DocumentBlock,
): TodoDocument => withBlocks(document, [...document.blocks, block]);
...
export const moveBlock = (
  ...
  if (index < 0 || target < 0 || target >= document.blocks.length)
    return document;
...
export const updateListTitle = (
  ...
  mapListBlock(document, listId, (list) => ({ ...list, title }));
...
export const updateItemText = (
  ...
  mapItem(document, listId, itemId, (item) => ({ ...item, text }));
...
export const setItemChecked = (
  ...
  mapItem(document, listId, itemId, (item) => ({ ...item, checked }));
