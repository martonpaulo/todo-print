...
describe("atomic list pagination", () => {
  it("moves a whole list to the next panel when it cannot fit", () => {
    ...
    expect(
      result.panels.map((panel) => panel.map((entry) => entry.id)),
    ).toEqual([["a"], ["b"]]);
...
describe("effective panel capacity", () => {
  ...
  it("gives an empty regular panel the same capacity after an automatic move and an explicit break", () => {
    ...
    const automatic = paginateBlocks(
      [list("filler"), list("a")],
      heights,
      options,
    );
    const explicit = paginateBlocks(
      [list("filler"), panelBreak("one"), list("a")],
      heights,
      options,
    );

    expect(
      automatic.panels.map((panel) => panel.map((entry) => entry.id)),
    ).toEqual([["filler"], ["a"]]);
    expect(
      explicit.panels.map((panel) => panel.map((entry) => entry.id)),
    ).toEqual([["filler"], ["a"]]);
...
describe("fixed three-slot page grouping", () => {
  const panelsOf = (count: number) =>
    Array.from({ length: count }, (_, index) => [list(`l${index}`)]);

  const shapeOf = (pages: ReturnType<typeof groupPanelsIntoPages>) =>
    pages.map((page) =>
      page.map((slot) =>
        slot === null ? null : slot.map((entry) => entry.id),
      ),
    );

  it.each([0, 1, 2, 3, 4, 7])(
    "lays %i panels out in whole pages of three slots",
    (panelCount) => {
      const pages = groupPanelsIntoPages(panelsOf(panelCount));

      expect(pages).toHaveLength(
        Math.max(1, Math.ceil(panelCount / PANELS_PER_PAGE)),
      );
      for (const page of pages) {
        expect(page).toHaveLength(PANELS_PER_PAGE);
      }
      expect(pages.flat().filter((slot) => slot !== null)).toHaveLength(
        panelCount,
      );
    },
  );
  ...
  it("fills only the trailing slots of the final page", () => {
    expect(shapeOf(groupPanelsIntoPages(panelsOf(1)))).toEqual([
      [["l0"], null, null],
    ]);
    expect(shapeOf(groupPanelsIntoPages(panelsOf(2)))).toEqual([
      [["l0"], ["l1"], null],
    ]);
    expect(shapeOf(groupPanelsIntoPages(panelsOf(3)))).toEqual([
      [["l0"], ["l1"], ["l2"]],
    ]);
