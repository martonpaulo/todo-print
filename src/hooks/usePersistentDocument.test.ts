...
const storedDocument = () => {
  ...
  return raw === null
    ? null
    : (JSON.parse(raw) as { blocks: { id: string }[] });
};

const renamedFirstList = (
  document: ReturnType<typeof createStarterDocument>,
  title: string,
) => ({
  ...document,
...
describe("usePersistentDocument", () => {
  ...
  it("reports a load failure and keeps unreadable stored content when decoding fails", () => {
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ version: 99, blocks: "broken" }),
    );
  ...
  it.each([
    ...
    "treats %s as unreadable stored content rather than an absent key",
    (_label, stored) => {
  ...
  it("never writes while the stored document is unreadable", () => {
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Draft"),
      ),
    );
  ...
  it("keeps the draft visible and reports a write failure when storing throws", () => {
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Unsaved"),
      ),
    );
  ...
  it("stays in the write-failed state across repeated failures", () => {
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "One"),
      ),
    );
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Two"),
      ),
    );
  ...
  it("clears the write failure only once a later write succeeds", () => {
    ...
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(createStarterDocument()),
    );
    const setItem = vi
      .spyOn(Storage.prototype, "setItem")
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Saved"),
      ),
    );
  ...
  it("keeps undo available for edits the browser refused to store", () => {
    ...
    act(() =>
      result.current.setDocument(renamedFirstList(original, "Unsaved")),
    );
    act(() => {
...
describe("usePersistentDocument history", () => {
  ...
  const editAfter = (
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, title),
      ),
    );
  };
  ...
  it("persists the replayed document through the same storage owner", () => {
    ...
    expect(storedDocument()).toEqual(
      JSON.parse(JSON.stringify(result.current.document)),
    );
  ...
  it("keeps a described edit out of the preceding coalesced step", () => {
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Renamed trimmed"),
        {
          kind: "task-removed",
  ...
  it("keeps the edit that follows a described one out of its step", () => {
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Removed"),
        {
          kind: "list-removed",
          subject: "List 2",
        },
  ...
  it("keeps a replacement out of the step the edit just before it opened", () => {
    ...
    act(() =>
      result.current.replaceDocument(
        renamedFirstList(result.current.document, "Imported"),
      ),
    );
  ...
  it("describes no removal for a replacement, so no recovery action is offered", () => {
    ...
    act(() =>
      result.current.setDocument(
        renamedFirstList(result.current.document, "Trimmed"),
        {
          kind: "task-removed",
          subject: "Task 1: Draft",
        },
  ...
  it("reports the removal an edit describes until the next transition", () => {
    ...
    expect(result.current.lastEdit).toEqual({
      kind: "task-removed",
      subject: "Task 1: Draft",
    });
