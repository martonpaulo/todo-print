...
const persistUnlessUnreadable = (
  ...
  status === "load-failed" ? "load-failed" : persistDocument(document);
...
export const usePersistentDocument = () => {
  ...
  const record = useCallback(
    (
      ...
      const coalesces =
        !isolated &&
        openWindow !== null &&
        now - openWindow < HISTORY_COALESCE_MS;
      ...
      apply(
        nextDocument,
        persistUnlessUnreadable(nextDocument, statusRef.current),
        edit,
      );
    },
    [apply],
  ...
  const traverse = useCallback(
    (from: TodoDocument[], to: TodoDocument[]) => {
      ...
      apply(
        restored,
        persistUnlessUnreadable(restored, statusRef.current),
        null,
      );
      return true;
    },
