...
const panelBreakDocument = (
  ...
  typography,
  blocks: Array.from({ length: panelCount }, (_, index) => index).flatMap(
    (index) => [
      ...(index === 0
        ? []
        : ([{ kind: "panel-break", id: `break-${index}` }] as const)),
      {
        kind: "list",
        id: `list-${index + 1}`,
        title: `Panel ${index + 1}`,
...
describe("printed page geometry", () => {
  ...
  const measurePrintedPaper = async (): Promise<Paper[]> => {
    ...
    if (boxes.length === 0)
      throw new Error("The generated PDF declares no /MediaBox");
    return boxes;
  };
  ...
  const render = async (
    ...
    await page.evaluate(
      (key: string, document: string) =>
        window.localStorage.setItem(key, document),
      STORAGE_KEY,
   ...
   */
  const measureLongRun = async (): Promise<{
    overflowMm: number;
    listHeightMm: number;
  }> =>
    page
      .evaluate(() => {
        ...
        const panel = window.document.querySelector(
          ".preview-stage .print-panel",
        );
        const list = window.document.querySelector(
          ".preview-stage .print-list",
        );
        if (!panel || !list)
          throw new Error("The long-run document rendered no panel");
        ...
        const boxes = [
          ...panel.querySelectorAll(
            ".moon-word, .print-list__title, .print-task__text",
          ),
        ];
   ...
   */
  const measureStrike = async (): Promise<{
    decoration: string;
    paintedMm: number;
  }> =>
    page
      .evaluate(() => {
        const text = window.document.querySelector(
          ".preview-stage .print-task__text--checked",
        );
        if (!text)
          throw new Error(
            "The checked-task document rendered no completed task",
          );
        ...
        return {
          ...
          paintedPx:
            paintedStyle.backgroundImage === "none"
              ? 0
              : Number.parseFloat(height),
        };
      })
      .then(({ decoration, paintedPx }) => ({
        decoration,
        paintedMm: pxToMm(paintedPx),
      }));
  ...
  const measureStrokes = async (): Promise<StrokeWidths> =>
    page
      .evaluate(() => {
        ...
        const rule = window.document.querySelector(
          ".preview-stage .print-list__rule",
        );
        const checkbox = window.document.querySelector(
          ".preview-stage .print-checkbox",
        );
        if (!panel || !rule || !checkbox) {
          throw new Error(
            "The stroke document rendered no divider, rule or checkbox",
          );
        }

        return {
          dividerPx: Number.parseFloat(
            window.getComputedStyle(panel, "::after").width,
          ),
  ...
  const measureMoonGeometry = async (): Promise<MoonGeometry> =>
    page.evaluate((alphabet: string) => {
      const title = window.document.querySelector(
        ".preview-stage .print-list__title .moon-text",
      );
      const task = window.document.querySelector(
        ".preview-stage .print-task__text--moon .moon-text",
      );
      if (!title || !task)
        throw new Error("The Moon geometry document did not render its text");

      const groups = [...title.querySelectorAll<SVGGElement>(".moon-word g")];
      if (groups.length !== alphabet.length) {
        throw new Error(
          `Expected ${alphabet.length} Moon glyphs, rendered ${groups.length}`,
        );
      }
      ...
      const glyphStyle = window.getComputedStyle(
        task.querySelector(".moon-text__glyphs") ?? task,
      );
      ...
      return {
        ...
        taskLineCount: Math.round(
          task.getBoundingClientRect().height / taskLineHeight,
        ),
        wordSpacingPx:
          glyphStyle.wordSpacing === "normal"
            ? 0
            : Number.parseFloat(glyphStyle.wordSpacing),
      };
    }, MOON_ALPHABET);
  ...
  const forShape = (panelCount: number): Measurement => {
    ...
    if (!result)
      throw new Error(`No measurement was collected for ${panelCount} panels`);
    return result;
  };
  ...
  beforeAll(async () => {
    ...
    for (const panelCount of PANEL_COUNTS) {
      ...
      measured.set(panelCount, {
        sheets: await measureSheets(),
        paper: await measurePrintedPaper(),
      });
    }
  ...
  const expectSequentialPanels = (sheets: MeasuredSheet[]) => {
    sheets.forEach((sheet) => {
      ...
      sheet.panels.forEach((panel, index) => {
        ...
        expect(panel.offsetLeftMm).toBeCloseTo(
          index * contract.panelWidthMm,
          1,
        );
      });
      expect(sheet.widthMm).toBeCloseTo(
        sheet.panels.length * contract.panelWidthMm,
        1,
      );
    });
  };
  ...
  it("states a contract whose panels tile the sheet", () => {
    expect(contract.panelsPerPage * contract.panelWidthMm).toBe(
      contract.pageWidthMm,
    );
  ...
  it("measured every document shape", () => {
    ...
    PANEL_COUNTS.forEach((panelCount) => {
      ...
      expect(sheets).toHaveLength(
        Math.ceil(panelCount / contract.panelsPerPage),
      );
  ...
  it("turns that run into height pagination can see", () => {
    ...
    expect(longRunMoon.listHeightMm).toBeGreaterThan(
      longRunLatin.listHeightMm / 2,
    );
  ...
  it("draws the panel divider, the list rule and the checkbox frame at one weight", () => {
    ...
    expect(
      Math.abs(measuredStrokes.ruleMm - measuredStrokes.dividerMm),
    ).toBeLessThanOrEqual(LAYOUT_UNIT_MM);
    expect(
      Math.abs(measuredStrokes.checkboxMm - measuredStrokes.dividerMm),
    ).toBeLessThanOrEqual(LAYOUT_UNIT_MM);
  });
  ...
  it("keeps the printed geometry identical when the document uses Moon typography", () => {
    ...
    if (!measuredMoon)
      throw new Error("No Moon-typography measurement was collected");
  ...
  it("keeps every Moon glyph inside its fixed SVG cell", () => {
    ...
    geometry.glyphBounds.forEach((bounds) => {
      expect(
        ...
        `${bounds.letter} has a finite left bound`,
      ).toBe(true);
      expect(
        Number.isFinite(bounds.minY),
        `${bounds.letter} has a finite top bound`,
      ).toBe(true);
      expect(
        bounds.minX,
        `${bounds.letter} stays inside the left cell edge`,
      ).toBeGreaterThanOrEqual(0);
      expect(
        bounds.minY,
        `${bounds.letter} stays inside the top band edge`,
      ).toBeGreaterThanOrEqual(0);
      expect(
        bounds.maxX,
        `${bounds.letter} stays inside the right cell edge`,
      ).toBeLessThanOrEqual(bounds.advance);
      expect(
        bounds.maxY,
        `${bounds.letter} stays inside the bottom band edge`,
      ).toBeLessThanOrEqual(bounds.bandHeight);
    });
  });
  ...
  it("uses one Moon line-height and an explicit word gap in titles and tasks", () => {
    ...
    expect(geometry.titleLineHeightRatio).toBeCloseTo(
      geometry.taskLineHeightRatio,
      2,
    );
  ...
  it.each(PANEL_COUNTS)(
    "renders %i panels as sequential slots of full-height paper",
    (panelCount) => {
      const { sheets, paper } = forShape(panelCount);
      ...
      paper.forEach((sheet, index) => {
        ...
        expect(sheet.widthMm).toBeCloseTo(
          sheets[index].panels.length * contract.panelWidthMm,
          0,
        );
      });
    },
  ...
  it("keeps that geometry under print media, where the stylesheet overrides it", () => {
    ...
    printMediaSheets[0].panels.forEach((panel) => {
      ...
      expect(panel.heightMm).toBeGreaterThanOrEqual(
        recordedClamp.minHeightMm - EPSILON_MM,
      );
      expect(panel.heightMm).toBeLessThanOrEqual(
        recordedClamp.maxHeightMm + EPSILON_MM,
      );
    });
    expect(printMediaSheets[0].heightMm).toBeCloseTo(clamp.minHeightMm, 1);
    expect(printMediaSheets[0].heightMm).toBeGreaterThanOrEqual(
      recordedClamp.minHeightMm - EPSILON_MM,
    );
    expect(printMediaSheets[0].heightMm).toBeLessThanOrEqual(
      recordedClamp.maxHeightMm + EPSILON_MM,
    );
  });

  it("lifts the print emulation afterwards", () => {
    expect(sheetsAfterRestore[0].heightMm).toBeCloseTo(
      contract.pageHeightMm,
      1,
    );
  });
  ...
  it("prints every sheet at the recorded page size with the recorded panel slots", () => {
    ...
    const sheetWidths = shapes.flatMap((m) =>
      m.sheets.map((s) => round(s.widthMm)),
    );
    const slotCounts = shapes.flatMap((m) =>
      m.sheets.map((s) => s.panels.length),
    );
    const paperWidths = shapes.flatMap((m) =>
      m.paper.map((s) => round(s.widthMm)),
    );
