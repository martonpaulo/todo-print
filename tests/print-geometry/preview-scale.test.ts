...
describe("preview scaling", () => {
  ...
  const measureStage = async (): Promise<StageMeasurement> =>
    page.evaluate(() => {
      ...
      if (!(shell instanceof HTMLElement))
        throw new Error("The preview rendered no sheet shell");
      if (!(sheet instanceof HTMLElement))
        throw new Error("The preview rendered no sheet");
  ...
  const at = (width: number): StageMeasurement => {
    ...
    if (!measurement)
      throw new Error(`No measurement was collected at ${width}px`);
    return measurement;
  };
  ...
  it.each(VIEWPORT_WIDTHS)(
    "scales the sheet to the rendered content box at %ipx",
    (width) => {
  ...
  it.each(VIEWPORT_WIDTHS)(
    "keeps the sheet inside the stage at %ipx",
    (width) => {
      const stage = at(width);

      expect(stage.sheetWidthPx).toBeLessThanOrEqual(
        stage.contentWidthPx + EPSILON_PX,
      );
      expect(stage.scrollWidthPx).toBeLessThanOrEqual(
        stage.clientWidthPx + EPSILON_PX,
      );
    },
  );

  it.each([900, 560, 520, 380])(
    "fills the whole content box at %ipx",
    (width) => {
      const stage = at(width);
  ...
  it("caps the scale at 1 on a viewport wider than the sheet", () => {
    ...
    expect(at(2600).contentWidthPx).toBeGreaterThan(
      at(2600).unscaledSheetWidthPx,
    );
  });
  ...
  describe("zoom", () => {
    const clickZoom = async (name: string): Promise<void> => {
      await page.evaluate((label: string) => {
        const control = [
          ...window.document.querySelectorAll(".preview-zoom button"),
        ].find(
          (button) =>
            (
    ...
    const settle = async (expected: number): Promise<void> => {
      await page.waitForFunction(
        (scale: number) => {
          ...
          return (
            Math.abs(
              new DOMMatrixReadOnly(window.getComputedStyle(sheet).transform)
                .a - scale,
            ) < 1e-6
          );
        },
    ...
    it("scrolls the stage to both edges of a sheet wider than it", async () => {
      ...
      const reach = await page.evaluate(() => {
        ...
        return {
          overflows: stage.scrollWidth > stage.clientWidth,
          startGap,
          endGap,
        };
      });
    ...
    it("reaches neither the controls nor the sheet under print media", async () => {
      ...
      try {
        ...
        expect(printed.sheetWidthPx).toBeCloseTo(
          printed.unscaledSheetWidthPx,
          0,
        );
      } finally {
    ...
    it("leaves the fit scale itself unchanged, so reset returns to it", async () => {
      ...
      expect(reset.scrollWidthPx).toBeLessThanOrEqual(
        reset.clientWidthPx + EPSILON_PX,
      );
    });
  });
});
