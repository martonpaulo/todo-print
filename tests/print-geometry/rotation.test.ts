...
describe("rotated print geometry", () => {
  ...
  let previewBoxes: {
    shell: { widthPx: number; heightPx: number };
  ...
  const measureSheet = async (): Promise<PrintedSheet> => {
    const raw = await page.evaluate(() => {
      ...
      if (!(sheet instanceof HTMLElement))
        throw new Error("The rotated document rendered no sheet");
      if (!(toolbar instanceof HTMLElement))
        throw new Error("The editor rendered no toolbar");
      ...
      return {
        ...
        panels: [...sheet.querySelectorAll<HTMLElement>(".print-panel")].map(
          (panel) => ({
  ...
  it("paints the turned sheet inside the portrait paper", () => {
    ...
    expect(sheet.paintedWidthMm).toBeLessThanOrEqual(
      contract.pageHeightMm + EPSILON_MM,
    );
