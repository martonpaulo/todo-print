...
describe("toolbar date control layout", () => {
  ...
  const measureToolbar = async (): Promise<ToolbarMeasurement> =>
    page.evaluate(() => {
      ...
      if (!(group instanceof HTMLElement))
        throw new Error("The toolbar rendered no group");

      const toggle = group.querySelector(".toggle-control");
      const date = group.querySelector(".date-control");
      if (!(toggle instanceof HTMLElement))
        throw new Error("The toolbar group rendered no toggle");
      if (!(date instanceof HTMLElement))
        throw new Error("The toolbar group rendered no date control");
  ...
  it.each(ROW_SHARING_WIDTHS)(
    "keeps the date control beside its toggle at %ipx",
    (width) => {
      const toolbar = at(width);

      expect(
        Math.abs(toolbar.dateCenterY - toolbar.toggleCenterY),
      ).toBeLessThanOrEqual(1);
    },
  );

  it.each(ROW_SHARING_WIDTHS)(
    "leaves the date control narrower than its group at %ipx",
    (width) => {
      const toolbar = at(width);

      expect(toolbar.dateWidthPx).toBeLessThan(
        toolbar.groupContentWidthPx - EPSILON_PX,
      );
    },
  );
